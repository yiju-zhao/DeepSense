...
export const DailyPaper: React.FC = () => {
  const [selectedTab, setSelectedTab] = React.useState(0);
  const [conferences, setConferences] = useState<Conference[]>([]);
  const [conferenceStats, setConferenceStats] =
    useState<ConferenceStats | null>(null);
  ...
  const renderOverview = () => {
    ...
    const stats = [
      ...
      {
        label: "Total Papers",
        value: conferenceStats.total_papers.toLocaleString(),
      },
      ...
      <PaperGrid container spacing={3}>
        {papers.map((paper) => (
          <Grid item xs={12} sm={6} md={4} key={paper.id}>
            <PaperCard>
              <PaperImage
                src={`/assets/img/paper/paper${parseInt(paper.id) % 3}.svg`}
                alt={paper.title}
              />
              <PaperContent>
        ...
        {conferences.map((conference) => (
          <Grid item xs={12} md={6} lg={4} key={conference.id}>
            <ConferenceCard>
              ...
              <ConferenceStats>
                Total Papers: {conference.totalPapers.toLocaleString()}
                <br />
                Average Citation: {conference.averageCitation}
                <br />
                Impact Score: {conference.impactScore}
                <br />
                Acceptance Rate: {conference.acceptanceRate}
                <br />
                Next Deadline:{" "}
